...
const cache = new Map(), entityCache = new Map();

export async function initDatabase(){
  try {
    // Use an absolute URL derived from this module. This avoids problems with
    // GitHub Pages base paths and does not append a query string to manifest.json.
    ...
    throw new Error(`Não foi possível carregar o banco em ${MANIFEST}. ${err?.message||err}`);
  }
}
...
function flattenManifest(){
  ...
  const add=(x,inherited={})=>{
    ...
    if(x.file||x.path||x.url||x.id||x.name){
      const e={...inherited,...x};
      if(e.type)e.type=normType(e.type);
      out.push(e);
    }
    for(const [k,v] of Object.entries(x)){
      if(["file","path","url"].includes(k))continue;
      if(typeof v==="object"){
        const typeKeys=["class","classes","subclass","subclasses","race","races","species","background","backgrounds","spell","spells","feat","feats","item","items","classfeature","classfeatures","subclassfeature","subclassfeatures"];
        add(v,{...inherited,...(typeKeys.includes(String(k).toLowerCase())?{type:normType(k)}:{})});
      }
    }
  };
  if(Array.isArray(catalog.entities))add(catalog.entities);
  else if(catalog.entities)add(catalog.entities);
  for(const k of ["classes","subclasses","races","species","backgrounds","spells","feats","items","files"])
    if(catalog[k])add(catalog[k],{type:normType(k)});
  return out;
}
function normalize(e,inherited={}){
  const x={...inherited,...e};
  x.type=normType(x.type||x.category||x.kind);
  x.id=x.id||`${x.type}:${x.name||x.file||x.path||Math.random()}`;
  return x;
}
export function manifestEntries(){
  if(!entriesCache) entriesCache=flattenManifest().map(x=>normalize(x));
  return entriesCache;
}
...
export async function loadEntity(e){
  if(!e)return null;
  if(entityCache.has(e.id))return entityCache.get(e.id);
...
function arrays(j){
  ...
  for(const [k,v] of Object.entries(j))
    if(Array.isArray(v))v.forEach(x=>{if(x&&typeof x==="object")out.push({...x,__type:normType(k)})});
  return out;
}
...
export async function findClassFeatures(cls,level){
  const rec=await recordsForEntity(cls);
  return rec.filter(x=>x.__type==="classFeature"&&Number(x.level||0)<=level).sort((a,b)=>Number(a.level||0)-Number(b.level||0));
}
export async function findSubclassFeatures(sub,level){
  const rec=await recordsForEntity(sub);
  return rec.filter(x=>x.__type==="subclassFeature"&&Number(x.level||0)<=level).sort((a,b)=>Number(a.level||0)-Number(b.level||0));
}
export async function getRecordArrays(e){return recordsForEntity(e)}
export function stats(){
  const all=manifestEntries();
  return {entities:all.length,official:all.filter(x=>!sourceIsHomebrew(x)).length,homebrew:all.filter(sourceIsHomebrew).length};
}
